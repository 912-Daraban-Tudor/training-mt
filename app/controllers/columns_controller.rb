class ColumnsController < ApplicationController
  ...
  def update
    respond_to do |format|
      if @column.update(column_params)
        format.html { redirect_to @column, notice: "Column was successfully updated.", status: :see_other }
  ...
  def destroy
    ...
    respond_to do |format|
      format.html { redirect_to columns_path, notice: "Column was successfully destroyed.", status: :see_other }
      format.json { head :no_content }
    end
  end

  private
    def order_column(direction)
    ...
    def column_params
      params.require(:column).permit(:name, :position, :status)
    end
end
