...
class ColumnsControllerTest < ActionDispatch::IntegrationTest
  ...
  test "should create column" do
    assert_difference("Column.count") do
      post columns_url, params: { column: { name: @column.name, position: @column.position, status: @column.status } }
    end
  ...
  test "should update column" do
    patch column_url(@column), params: { column: { name: @column.name, position: @column.position, status: @column.status } }
